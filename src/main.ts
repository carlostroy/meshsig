...
function parseStartArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--gateway': case '-g': config.gatewayUrl = args[++i]; break;
...
async function cmdStart() {
  ...
  const server = new MeshServer({ port, host, dbPath, name: config.name, peers: config.peers || [], gatewayUrl });
  const terminal = new TerminalDisplay();

  await server.start();

  const proxyMode = !!gatewayUrl;
...
${proxyMode ? `
    ${GREEN}●${RESET} ${DIM}MODE${RESET}        ${YELLOW}${BOLD}PROXY${RESET} → ${CYAN}${gatewayUrl}${RESET}
    ${DIM}    All agent traffic is intercepted, signed, and forwarded${RESET}
` : ''}
${DIM}    ─────────────────────────────────────────────────────────${RESET}
...
${BOLD}SERVER OPTIONS${RESET} (meshsig start)
  ...
  ${DIM}--gateway, -g <url>${RESET} Proxy mode: intercept & sign traffic to gateway
...
${BOLD}EXAMPLES${RESET}
  ...
  ${GREEN}# Start as proxy (recommended)${RESET}
  meshsig start --gateway http://localhost:3001

  ${GREEN}# Start standalone${RESET}
